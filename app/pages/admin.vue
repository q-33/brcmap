      ...
      <div v-if="usage?.available" class="mt-3 space-y-1.5 text-xs text-(--ui-text-muted)">
        <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span class="flex items-center gap-1.5">
            ...
            on the site now<template v-if="usage.activePaths.length"> ({{ usage.activePaths.map(p => `${p.path} ${p.n}`).join(', ') }})</template>
          </span>
          <span class="text-(--ui-text-muted)/50">·</span>
          <span><span class="font-medium text-(--ui-text)">{{ usage.last24h }}</span> in 24h</span>
          <span class="text-(--ui-text-muted)/50">·</span>
          <span><span class="font-medium text-(--ui-text)">{{ usage.last7d }}</span> in 7 days</span>
          <span v-if="usage.peak" class="text-(--ui-text-muted)/70">· peak {{ usage.peak.n }} at {{ new Date(usage.peak.at).getHours() }}:00</span>
